 ...
 * Main landing page for students. Displays summary stats, pending tasks,
 * today's schedule, and active courses.
 */
export const StudentDashboard = () => {
    ...
    useEffect(() => {
        api.get<DashboardData>('/student/dashboard')
            ...
            .catch((err: unknown) => console.error("Dashboard Error:", err))
            .finally(() => setLoading(false));
    }, []);

    if (loading) return <div className="p-20 text-center text-slate-500">Loading your dashboard...</div>;
    if (!data) return null;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                <div className={`col-span-1 lg:col-span-2 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden bg-gradient-to-r ${theme.gradient}`}>
                    <div className="relative z-10">
                        <h1 className="text-3xl font-black mb-2">Hello, {user?.fullName.split(' ')[0]}! 👋</h1>
                        <p className="text-blue-100 text-lg opacity-90 mb-6">
                            You have <strong className="text-white">{data.stats.pendingCount} pending tasks</strong> for this week.
                        </p>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-center items-center relative overflow-hidden group hover:border-blue-200 transition-all">
                    ...
                    <span className="text-sm font-bold text-slate-400 uppercase tracking-wider">GPA Average</span>
                </div>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-center items-center relative overflow-hidden group hover:border-purple-200 transition-all">
                    ...
                    <span className="text-sm font-bold text-slate-400 uppercase tracking-wider">Active Courses</span>
                </div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">

                {/* 2. LEFT COLUMN: Schedule & Tasks */}
                <div className="space-y-6 lg:col-span-1">
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                        ...
                        {data.todayClasses.length === 0 ? (
                            <p className="text-sm text-slate-400 italic">No classes today. Enjoy your day off! 🎉</p>
                        ) : (
                                ...
                                {data.todayClasses.map((cls, idx) => (
                                    <div key={idx} className="flex gap-3 items-start p-3 rounded-xl bg-slate-50 border border-slate-100">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-500 mt-1">Room: {cls.classroom}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        {data.pendingTasks.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {data.pendingTasks.map(task => (
                                    <div
                                        key={task.id}
                                        // Navigate passing activity ID in state to auto-open modal
                ...
                {/* 3. RIGHT COLUMN: Active Courses */}
                <div className="lg:col-span-2">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="font-bold text-slate-800 text-xl flex items-center gap-2">
                            <BookOpen className="text-blue-600" /> My Courses
                        </h3>
                        <Link to="/dashboard/subjects" className="text-xs font-bold text-blue-600 hover:underline">View All</Link>
                    </div>
                        ...
                        {data.courses.map(course => (
                            <Link to={`/dashboard/course/${course.id}`} key={course.id}
                                ...
                                <div className="flex justify-between text-xs font-bold text-slate-400">
                                    ...
                                    {course.average > 0 && <span className="text-green-600">Grade: {course.average}</span>}
                                </div>
                            </Link>
                        ))}
                    </div>
