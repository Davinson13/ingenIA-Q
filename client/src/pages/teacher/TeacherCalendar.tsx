 ...
 * Displays the teacher's monthly schedule, including classes, tutoring sessions,
 * academic events, and personal extracurricular activities.
 */
export const TeacherCalendar = () => {
    ...
    const fetchEvents = useCallback(async () => {
        ...
        try {
            ...
            const res = await api.get<AgendaEvent[]>(`/teacher/calendar?month=${month}&year=${year}`);
            setEvents(res.data);
        } catch (error) {
    ...
    /**
     * Handles creation of a personal/extracurricular event.
     */
    ...
    const renderCalendarDays = () => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const dayEvents = events.filter(e => {
                ...
                // Note: Using local date components to match visual calendar grid
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-20">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-4">
                    <div>
                        <h1 className="text-3xl font-black flex items-center gap-3">
                            <CalIcon size={32} /> My Agenda
                        </h1>
                        <p className="text-indigo-100 opacity-90">Manage Classes, Tutoring, and Activities.</p>
                    </div>
            ...
            <div className="flex flex-wrap gap-4 justify-center text-xs font-bold text-slate-600">
                ...
                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-orange-500"></span> Tasks/Exams</span>
                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-purple-500"></span> Tutoring</span>
                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-green-500"></span> Extracurricular</span>
            </div>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 animate-in zoom-in-95">
                        <h3 className="text-xl font-bold text-slate-800 mb-4">Add Extracurricular Activity</h3>
                        <form onSubmit={handleCreateExtra} className="space-y-4">
                            <div>
                                ...
                                <input required type="text" className="w-full p-3 border rounded-lg outline-none focus:ring-2 focus:ring-indigo-500"
                                    value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="Ex: Board Meeting" />
                            </div>
                            <div>
                            ...
                            <div className="flex gap-2 pt-2">
                                <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-3 text-slate-500 hover:bg-slate-100 rounded-lg font-bold">Cancel</button>
                                <button type="submit" className="flex-1 py-3 bg-indigo-600 text-white rounded-lg font-bold hover:bg-indigo-700 shadow-lg">Save</button>
                            </div>
                        </form>
                    </div>
                </div>
